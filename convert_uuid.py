...
for table in ["messages", "channels", "users", "emojis", "sync_servers"]:
    for row in cur.execute(f"SELECT uuid FROM {table}"):
        uuid, = row
        if uuid > 2**53-1:
            new_uuid = uuid >> (63 - 53)
            cur.execute(f"update {table} set uuid={new_uuid} where uuid={uuid}")

for row in cur.execute("SELECT uuid,author_uuid,channel_uuid FROM messages"):
...
for row in cur.execute("SELECT user_uuid FROM sync_data"):
    uuid, = row
    if uuid > 2**53-1:
        new_uuid = uuid >> (63 - 53)
        cur.execute(f"update sync_data set user_uuid={new_uuid} where user_uuid={uuid}")

for row in cur.execute("SELECT user_uuid FROM sync_servers"):
    uuid, = row
    if uuid > 2**53-1:
        new_uuid = uuid >> (63 - 53)
